...
use crate::models::{
    Query, QueryType, QueryResponse, IslamicPrinciple, TokenAnalysis, AnalysisStatus,
    ScrapedData, ScrapedDataType, HistoryQuery, BacktestResult,
};
use crate::actors::{spawn_query_actor, spawn_scraper_actor, spawn_analyzer_actor, spawn_history_actor};
...
async fn test_gherkin_scenario_text_input_btc() {
    ...
    assert!(
        result.follow_up_questions.iter().any(|q| q.contains("Fatwa") || q.contains("negara lain")),
...
async fn test_gherkin_scenario_audio_input_sol() {
    ...
    let query = Query::new_audio(
        mock_audio_data,
        Some("gherkin_user_2".to_owned()),
        Some("id".to_owned())
    );
    ...
    assert!(
        result.follow_up_questions.iter().any(|q| 
            q.to_lowercase().contains("dapps") || 
...
async fn test_gherkin_scenario_chatbot_usdt() {
    ...
    let query = Query::new_text(
        "Apakah $USDT halal?".to_owned(),
        Some("chatbot_user".to_owned()),
        Some("id".to_owned())
    );
    ...
    let response_lower = result.response.to_lowercase();
    assert!(
        response_lower.contains("usdt") || 
        response_lower.contains("stablecoin") || 
        response_lower.contains("fiat"),
        "Should explain USDT as fiat-backed stablecoin"
    );

    assert!(
        response_lower.contains("riba") || 
        response_lower.contains("halal") || 
        response_lower.contains("mubah"),
...
async fn test_gherkin_scenario_automatic_scraping() {
    ...
    for url in test_urls {
        let result = timeout(
            ...
            scraper_handle.scrape_single_url(url.to_owned())
        ).await;
        ...
        match result {
            Ok(Ok(scraped_data)) => {
                ...
                assert!(matches!(scraped_data.data_type, ScrapedDataType::CryptoHalalVerification | ScrapedDataType::IslamicFinanceContent));
            }
            Ok(Err(_)) => {
...
async fn test_gherkin_scenario_analysis_history() {
    ...
    for token in tokens {
        let query = Query::new_token_ticker(
            token.to_owned(),
            Some(user_id.clone()),
            Some("id".to_owned())
        );
    ...
    let history = timeout(
        ...
        history_handle.query_analyses(history_query)
    )
    .await
...
async fn test_gherkin_scenario_backtest_evaluation() {
    ...
    let query = Query::new_token_ticker(
        "BTC".to_owned(),
        Some("backtest_user".to_owned()),
        Some("id".to_owned())
    );

    let initial_result = query_handle.process_query(query).await.expect("Initial analysis should succeed");
    ...
    // Run backtest
    let backtest_result = timeout(
        Duration::from_secs(10),
        analyzer_handle.run_backtest(analysis_id)
    )
    .await
...
async fn test_gherkin_scenario_follow_up_questions() {
    ...
    // Initial query
    let query = Query::new_text(
        "Apakah Bitcoin halal?".to_owned(),
        Some("followup_user".to_owned()),
        Some("id".to_owned())
    );
    ...
    let follow_up_result = timeout(
        Duration::from_secs(6),
        query_handle.process_query(follow_up_query)
    )
    .await
...
async fn setup_test_system() -> (
    ...
    let query_handle = spawn_query_actor(
        scraper_handle.clone(),
